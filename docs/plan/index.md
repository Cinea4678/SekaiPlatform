...
- 涉及内部服务调用、内部 endpoint、维护接口或跨服务用户上下文传递的改动，必须遵循 @../design/security-model.md；不得新增绕过内部 token 模型的临时鉴权分支。
...
- 实现外部数据源适配器，优先支持 Moe Sekai / Exmeaning master JSON 和资源镜像。
- 实现 master JSON 解析。
- 实现 `story_groups -> stories -> story_source_lines` 数据入库。
- 实现 scenario URL 生成。
- 实现 scenario JSON 下载。
- 实现 Unity scenario JSON 解析为原文行。
- 覆盖活动剧情、主线剧情、卡面剧情、区域对话和特殊剧情的阶段四同步范围。
- 实现同步日志。
- 实现每天一次自动同步。
- 实现管理员手动同步。
...
- 能同步活动剧情、主线剧情、卡面剧情、区域对话和特殊剧情。
- 同步成功后数据库存在剧情集、剧情和原文行。
- 活动剧情按活动生成剧情集，并按章节生成剧情。
- 主线剧情按 unit 生成剧情集，并按章节生成剧情。
- 卡面剧情按卡牌生成剧情集，并按前篇/后篇生成剧情。
- 区域对话按分类生成剧情集，并按 actionSet 对话生成剧情。
- 同步失败记录错误日志。
- 同步失败不自动重试。

完成记录：见 [Phase 4 完成记录](phase-4-status.md)。
...
完成记录：见 [Phase 5 完成记录](phase-5-status.md)。
...
完成记录：见 [Phase 6 完成记录](phase-6-status.md)。
...
完成记录：见 [Phase 7 完成记录](phase-7-status.md)。
...
完成记录：见 [Phase 8 完成记录](phase-8-status.md)。
...
准备记录：见 [部署交付准备记录](phase-9-status.md)。
