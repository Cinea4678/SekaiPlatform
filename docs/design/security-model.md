...
- 合作伙伴开放 API 边界：外部合作伙伴只访问 OpenApiService，当前允许匿名访问，但暂不开放业务接口。
...
OpenApiService 当前只校验监听端口、未开放路径和限流规则，不调用内部业务服务。后续新增开放业务接口时，OpenApiService 调用内部服务必须继续签发内部 token，内部服务不得感知外部匿名调用方。
...
| 开放 API | OpenApiService 端口，当前无业务路径 | 匿名访问 |
